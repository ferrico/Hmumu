...
g++ ./plotter_vbfzll_bTagSys.C -g -o plot `root-config --cflags --glibs`   -lMLP -lXMLIO -lTMVA  
...
QCDcorrectionARRAY=(nom nom nom nom nom up down nom nom nom nom nom)
JEScorrectionARRAY=(nom nom nom up down nom nom nom nom nom nom nom)
bTagWeightJESARRAY=(nom up down nom nom nom nom nom nom nom nom nom)
JERcorrectionARRAY=(nom nom nom nom nom nom nom up down nom nom nom)
PUcorrectionARRAY=(nom nom nom nom nom nom nom nom nom up down nom)
WEIGHTcorrectionARRAY=(zero nom nom nom nom nom nom nom nom nom nom nom)
...
for i in $(seq 1 1 11); do QCDcorrection=${QCDcorrectionARRAY[$i]}; JEScorrection=${JEScorrectionARRAY[$i]}; bTagWeightJEScorrection=${bTagWeightJESARRAY[$i]};  JERcorrection=${JERcorrectionARRAY[$i]}; PUcorrection=${PUcorrectionARRAY[$i]}; WEIGHTcorrection=${WEIGHTcorrectionARRAY[$i]};
echo run_plotter_parallel_bTagSys.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection $JERcorrection $PUcorrection $WEIGHTcorrection;
source run_plotter_parallel_bTagSys.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection $JERcorrection $PUcorrection $WEIGHTcorrection&
done
...
# for i in $(seq 0 1 5 ); do QCDcorrection=${QCDcorrectionARRAY[$i]}; JEScorrection=${JEScorrectionARRAY[$i]};
# if [ "$i" -eq "0" ]; then
# for j in $(seq 0 1 2 ); do bTagWeightJEScorrection=${bTagWeightJESARRAY[$j]};
# echo run_plotter_parallel.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection;
# source run_plotter_parallel.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection &
# done
# else
# bTagWeightJEScorrection=${bTagWeightJESARRAY[0]};
# echo run_plotter_parallel.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection;
# source run_plotter_parallel.sh $QCDcorrection $JEScorrection $bTagWeightJEScorrection &
# fi
# done
