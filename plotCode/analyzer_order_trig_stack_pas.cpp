...
// DY_name = "DYJetstoLL_amc_Filter105";   // NLO inclusive
// DY_file_name = "DYJetstoLL_amc_Filter105";   // NLO inclusive
// DY_name = "DYJetstoLL_amc";   // NLO binned jets
// DY_file_name = "DYJetstoLL_amc";   // NLO binned jets
DY_name = "DYJetsToLL_M";   // LO
DY_file_name = "DYJetsToLL_M";   // LO
...
TString file_names_QCDdown_JESdown[nfiles] ={"SingleMuon","WJetsToLNu","WW","ZZ","WZ","ST","TT",DY_file_name,"VBF_HToMuMu", "GluGlu_HToMuMu"};
TString file_names_QCDup_JESup[nfiles] ={"SingleMuon","WJetsToLNu","WW","ZZ","WZ","ST","TT",DY_file_name,"VBF_HToMuMu", "GluGlu_HToMuMu"};
...
//change the path to the files 
	...
	file_names_QCDup_JESup[i] =   file_names[i];
	file_names_QCDdown_JESdown[i] = file_names[i];
//---------------------------------------------------------------------------------------------------------------------- OK
...
// 	}
		...
		file_names_QCDup_JESup[i].Append("_QCDScaleup_JESup_v25_reskim");
		file_names_QCDdown_JESdown[i].Append("_QCDScaledown_JESdown_v25_reskim");
	...
	file_names_QCDup_JESup[i].Append(".root");
	file_names_QCDdown_JESdown[i].Append(".root");
...
const int nhistos = 6; //79 //40//52
// TString hist_names[nhistos]={"hMqq", "hHll_mass", "hEtaQQ","hHTsoftEWK","hSoft_n2EWK","hSoft_n5EWK","hSoft_n10EWK","hPVs", "hJet1q_pt", 
// 							"hJet1q_eta", "hJet1q_phi", "hJet2q_phi", "hJet1q_ptd", "hJet1q_axis2", "hJet1q_mult", "hJet2q_pt", "hJet2q_eta", "hJet2q_ptd", "hJet2q_axis2", "hJet2q_mult", "hmet",   "hJet1q_leadTrackPt", "hJet2q_leadTrackPt", "hqq_pt", "hqgl", "hqgl2", "hHll_pt", "hHll_phi", "hHll_eta",  "hrho", "hlepton1_pt", "hlepton2_pt", "hlepton1_eta", "hlepton2_eta","hlepton1_iso03", "hlepton2_iso03", "hDeltaRelQQ", "hRptHard", "hEtaQQSum", "hPhiZQ1", "hHll_y", "hHll_ystar", "hHll_zstar", "hMqq_log", "hlheV_pt","hJet3_pt","hPhiQQ", "hJets12_pt","hJets12_pt_log", "hJet1q_pt_log", "hJet2q_pt_log", "hHT","hlheHT_log", "hlheNj", "hNAdJets", "hJet3_pt", "hsoftleadTrackPt", "hsoftleadTrackEta", "hAdJetHT", "hJet3_eta", "hJet3_pt_new", "hMaxJetBTagCSV","hVirtual_Pt1","hVirtual_Pt2", "hMaxJetBTagCMVA","hthetastar_W1","hthetastar_W2", "hMaxSecondJetBTagCMVA", "hMaxSecondJetBTagCSV","hVirtual_eta1","hVirtual_eta2", "hThetaStarJet", "hThetaPlanes", "hThetaStar","hDiffmass", "hThetaStarAbs","hTheta_HiggsJ1","hTheta_HiggsJ2","hthetastar_W2toHW1","hthetastar_W1toHW2","hthetastar_HtoWW", "hTotalEnergy","hTotalEnergylog","hVirtual_phi1","hParton_M1","hParton_M2","hVirtual_phi2","hWWmass", "hEnergy_fraction_Parton1","hPz","hPzAbs" ,"hVirtual_Wmass1","hVirtual_Wmass2", "hInvariant_Masslog","hInvariant_Mass" , "hEnergy_fraction_Parton2", "hBDT_VBF" , "hBDT_VBF_atanh", "hbdt"};

///////////////////////////////////////////////////////////////////////
// Il primo istogramma è quello di cui  vengono calcolati gli YIELDS //
TString hist_names[nhistos]={"hHll_mass", "hlepton1_pt", "hlepton2_pt",
							"hlepton1_eta", "hlepton2_eta", "hHll_eta"};//, 
// 								"hHTsoftEWK", "hInvariant_Mass", "hmet", "hBDTG_VBF",  "hMLP_VBF", 
// 								"hKNN_VBF", "hHll_eta"};//, "new BDT", "hBDTG_VBF", "hMLP_VBF"};
// Il primo istogramma è quello di cui  vengono calcolati gli YIELDS //
///////////////////////////////////////////////////////////////////////
...
do{
    ...
    for (int i=1;i<nfiles;i++) {
        ...
        if (counter_ratio==5) file_names_mc[i] = file_names_QCDup_JESup[i];
        if (counter_ratio==6) file_names_mc[i] = file_names_QCDdown_JESdown[i];
    ...
    do{
        ...
        if (files!=0) {
            ...
            if (file_name_tag.find(DY_name)!=std::string::npos)  DYint= histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1);
            	...
            	if(files < nfiles - 2) // gli ultimi due sono VBF e ggH
            		MCint+=histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1); 
            }

			std::cout<<dataInt<<"\t"<<file_name_tag<<"\t"<<DYint<<"\t"<<MCint<<"\t"<<histos_mc[hist]->Integral(0,histos_mc[hist]->GetNbinsX()+1)<<std::endl;
...
///////////////////////////////////
    ...
    std::cout<<tmp_ratio<<std::endl;
} while (counter_ratio<7);
//----------------------------------------------------------------------------------------------------------------------
...
////////////////////////////
        ...
        if ((hist == nhistos-3))  signalMultipyFactor = 20.;

	//	if (files==1) {
		if (files==nfiles-2) { // VBF
	...
	if (files==0) out_efficiency<<leg_names[order[files]]<<"\t \t \t"<< std::setprecision(8)<<histos[0]->Integral(0,histos[0]->GetNbinsX()+1) <<endl;
		...
		if(leg_names[order[files]] == "Z + jets"){
			out_efficiency<<leg_names[order[files]]<<"\t\t\t  "<<std::setprecision(8)<<histos[0]->Integral(0,histos[0]->GetNbinsX()+1)<<endl;
		}
...
// std::cout << "HEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"  << std::endl;
...
// 	std::cout << "\nHEREEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"  << std::endl;
...
//     if (hist_names[i].CompareTo("hHll_mass")!=0 || hist_names[i].CompareTo("hHll_mass_BB")!=0 || hist_names[i].CompareTo("hHll_mass_BE")!=0){
...
//     if ((hist_names[i].CompareTo("hBDT_VBF")!=0) && (hist_names[i].CompareTo("hBDT_VBF_atanh")!=0) && (hist_names[i].CompareTo("hHll_mass")!=0) && (hist_names[i].CompareTo("hHll_mass_BB")!=0) && (hist_names[i].CompareTo("hHll_mass_BE")!=0))
//         	data_histos2[i]->Draw("PEsame");
